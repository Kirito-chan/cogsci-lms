...
export default function createOrderedCommentsMap(commentsArray) {
  ...
  const originalComments = commentsArray.filter((c) => c.ref_comment_id == null);
  const refComments = commentsArray.filter((c) => c.ref_comment_id != null);
  
  for (const comment of originalComments) { commentsMapTemp.set(comment.id, []); }
  for (const comment of refComments) { commentsMapTemp.get(comment.ref_comment_id).push(comment);}
